...
ok( $schema1 instanceOf SQL_Schema, "Parse returns an SQL_Schema object" );

is( count($schema1->tables), 2, "Parsed two table" );
is( count($schema1->routines), 0, "Parsed no routines" );

$test_table = $schema1->tables['test'];
ok( $test_table instanceOf SQL_Table, "We got an SQL_Schema_Table object for the table test" );
...
ok( $column instanceOf SQL_Column, "Column id isa SQL_Column" );
is( $column->name, "id", "Column name set" );
is( count($column->aliases), 0, "No aliases" );
ok( $column->type instanceOf SQL_Integer, "Column type set" );
